...
export default function InlineNotes({ notes, onAdd, startDate, endDate, isDark }) {
  ...
  return (
    <div className="py-1">
      <h3 className={`text-[9px] font-bold uppercase tracking-[0.2em] mb-2 ${isDark ? "text-white/15" : "text-stone-400"}`}>
        Notes
      </h3>
      ...
      {lines.map((line, i) => (
        <input
          ...
          className={`ruled-line ${isDark ? "ruled-line-dark" : ""} w-full bg-transparent text-[11px] outline-none px-0 ${
            isDark ? "text-white/40 placeholder:text-white/10" : "text-stone-500 placeholder:text-stone-300"
          }`}
      ...
      {notes.length > 0 && (
        <div className="mt-2 space-y-0.5">
          {notes.slice(0, 3).map((note) => (
            <p key={note.id} className={`text-[9px] truncate ${isDark ? "text-white/10" : "text-stone-300"}`}>
              {note.text}
            </p>
          ))}
          {notes.length > 3 && (
            <p className={`text-[9px] font-medium ${isDark ? "text-violet-400/30" : "text-violet-400"}`}>+{notes.length - 3} more</p>
          )}
        </div>
      )}
    </div>
